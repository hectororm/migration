...
- `DbTracker::markApplied()` is now safe against concurrent processes: it uses an "ignore duplicates" insert and checks the affected-row count (1 = applied, 0 = already inserted by another process) instead of a non-atomic check-then-insert that could raise a primary-key violation when two runners marked the same migration at once
